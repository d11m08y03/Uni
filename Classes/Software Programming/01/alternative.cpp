...
struct Matrix {
  ...
  Matrix(size_t rows, size_t cols, std::vector<int> arr) : rows(rows), cols(cols), arr(arr) {}
  ...
  void print() const {
    ...
    std::for_each(view.begin(), view.end(), [](const auto& tuple) {
      if (std::get<0>(tuple) % 2 == 0) std::clog << '\n' << std::get<1>(tuple) << ' ';
      else std::clog << std::get<1>(tuple) << ' ';
    });
    std::clog << '\n';
  }
};
